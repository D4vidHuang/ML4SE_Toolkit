...
VS = 20  # minimum vertical separation between things. For a 3px stroke, must be at least 4
...
class Terminal(DiagramItem):
    def __init__(
        ...
        error_match = text.lower().strip().split(" error=")
        self.text = error_match[0]
        self.error_count = int(error_match[1]) if len(error_match) > 1 else 0
        self.href = href
        self.title = title
        self.cls = cls
        self.n_lines = len(self.text.split("\n"))
        line_length = max(len(line) for line in self.text.split("\n"))
        self.width = line_length * CHAR_WIDTH + 20
        self.up = 11
        self.down = 11 + (self.n_lines - 1) * LINE_HEIGHT
        self.needsSpace = True
        addDebug(self)
    ...
    def _get_color_from_error_count(self) -> str:
        max_error = 1000
        normalized_error = min(self.error_count / max_error, 1)
        red = int(255 * normalized_error)
        green = int(255 * (1 - normalized_error))
        return f"rgb({red}, {green}, 0)"

    def format(self, x: float, y: float, width: float) -> Terminal:
        left_gap, right_gap = determineGaps(width, self.width)

        # Hook up the two sides if this is narrower than its stated width.
        Path(x, y).h(left_gap).addTo(self)
        Path(x + left_gap + self.width, y).h(right_gap).addTo(self)

        fill_color = self._get_color_from_error_count()
        DiagramItem(
            "rect",
            {
                "x": x + left_gap,
        ...
        y_offset = y - ((self.n_lines - 1) * LINE_HEIGHT / 2)
        for i, line in enumerate(self.text.split("\n")):
            text_item = DiagramItem(
                "text", {"x": x + left_gap + self.width / 2, "y": y_offset + 4 + (LINE_HEIGHT * i)}, line
            )
            if self.href is not None:
                a = DiagramItem("a", {"xlink:href": self.href}, text_item)
                a.addTo(self)
            else:
                text_item.addTo(self)
...
class NonTerminal(DiagramItem):
    def __init__(
        ...
        self.title = title
        self.cls = cls
        self.width = len(self.text) * CHAR_WIDTH + 20
        self.up = 11
        self.down = 11
        self.needsSpace = True
        addDebug(self)
    ...
    def format(self, x: float, y: float, width: float) -> NonTerminal:
        ...
        if self.href is not None:
            a = DiagramItem("a", {"xlink:href": self.href}, text)
            a.addTo(self)
        else:
            text.addTo(self)
