...
export class Terminal extends FakeSVG {
	constructor(text, {href, title, cls} = {}) {
		...
		const errorMatch = text.match(/ error=(\d+)/);
        this.text = text.split(" error=")[0];
        this.errorCount = errorMatch ? parseInt(errorMatch[1], 10) : 0;

		this.href = href;
		this.title = title;
		this.cls = cls;
		this.num = (this.text.match(/\n/g) || []).length;
		this.width = this.text.length * Options.CHAR_WIDTH + 20;
		this.height = 0;
		this.up = 11;
		this.down = 17 * this.num + 11;
		this.needsSpace = true;
	...
	_getColorFromErrorCount() {
		const maxError = 1000;
		console.log(this.errorCount);
		const normalizedError = Math.min(this.errorCount / maxError, 1);
		console.log(normalizedError);
		const red = Math.floor(255 * normalizedError);
		console.log(red);
		const green = Math.floor(255 * (1 - normalizedError));
		console.log(green);
	...
	format(x, y, width) {
		...
		const color = this._getColorFromErrorCount();
